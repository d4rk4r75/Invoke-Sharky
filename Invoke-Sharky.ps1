function Invoke-Sharky {
  ...
  function Banner() {
    ...
    Write-Output "# Inspired by: https://github.com/absolomb/WindowsEnum"
    Write-Output "---------------------------------------------------------------------"
  }
  ...
  # function FindLocalAndProof {
  #   $filepaths = $(Get-ChildItem -Path C:\ -Recurse -ErrorAction SilentlyContinue | Where-Object { $_.Name -match "^(local|proof)\.txt$" })
  #   | ForEach-Object { Write-Host ""; "Path: " + $_.FullName; "Content: " + (Get-Content $_.FullName) }; Write-Host ""
  #   foreach ($fp in $filepaths) {
  #     Copy-Item -Path $fp -ItemType File
  #   }
  # }
